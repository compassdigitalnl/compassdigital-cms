...
export class ProvisioningService {
  ...
  async provision(input: ProvisioningInput): Promise<ProvisioningResult> {
    ...
    try {
      ...
      const { createRailwayDatabase } = await import('@/features/platform/integrations/railway')
      ...
      // try {
      //   const { initializeMeilisearchIndexes } = await import('@/features/search/lib/meilisearch/initializeIndexes')
  ...
  private async rollback(
    ...
    try {
      ...
      if (this.options.rollbackConfig?.deleteDatabase && databaseId) {
        const { deleteRailwayDatabase } = await import('@/features/platform/integrations/railway')
