...
export function SegmentCard({ segment, onEdit, onDuplicate, onDelete }: SegmentCardProps) {
  ...
  return (
    <div className={`rounded-lg border bg-white shadow-sm transition-shadow hover:shadow-md ${isArchived ? 'border-grey-light opacity-70' : 'border-grey-light'}`}>
      <div className="p-4">
        {/* Header */}
        <div className="flex items-start justify-between">
          <div className="min-w-0 flex-1">
            <h3 className="truncate text-base font-semibold text-navy">{segment.title}</h3>
            {segment.description && (
              <p className="mt-1 line-clamp-2 text-sm text-grey-mid">{segment.description}</p>
            )}
          </div>
        ...
        <div className="mt-4 flex items-center gap-6">
          <div>
            ...
            <span className="ml-1 text-sm text-grey-mid">abonnees</span>
          </div>
        ...
        {/* Actions */}
        <div className="mt-4 flex items-center gap-2 border-t border-grey-light pt-3">
          {onEdit && (
            <button
              type="button"
              onClick={onEdit}
          ...
          {onDuplicate && (
            <button
              type="button"
              onClick={onDuplicate}
              className="rounded px-3 py-1.5 text-sm font-medium text-grey-dark transition-colors hover:bg-grey-light"
            >
              Dupliceren
            </button>
          )}
