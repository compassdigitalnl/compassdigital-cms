...
export function CasesArchiveTemplate({
  cases,
  categories,
  totalPages,
  currentPage,
  ...
  const featuredImage = featuredCase
    ? typeof featuredCase.featuredImage === 'object'
      ? (featuredCase.featuredImage as Media)?.url
      : null
    : null
  const featuredCategory =
    featuredCase && typeof featuredCase.category === 'object'
      ? featuredCase.category?.title
      : null
    ...
    <div className="bg-white">
      <div className="mx-auto max-w-7xl px-6 py-4">
        <nav className="flex items-center gap-2 text-sm text-grey-mid">
          ...
          <span>/</span>
          <span className="text-navy">Cases</span>
        </nav>
      </div>

      <section className="bg-gradient-to-br from-secondary to-secondary/90 py-12 md:py-16">
        <div className="mx-auto max-w-7xl px-6 text-center">
          <h1 className="font-display text-3xl text-white md:text-4xl lg:text-5xl">Onze cases</h1>
          <p className="mx-auto mt-4 max-w-2xl text-lg text-white/80">
            Bekijk een selectie van onze succesvolle projecten en samenwerkingen.
          </p>
          <div className="mt-6 flex items-center justify-center gap-8">
            ...
            <div className="text-center">
              ...
              <div className="text-sm text-white/60">Specialisaties</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 md:py-16">
        {featuredCase && featuredImage && (
          <div className="mb-12">
            <FeaturedCard
              ...
              description={featuredCase.shortDescription || ''}
              imageUrl={featuredImage}
              imageAlt={featuredCase.title}
              badge={featuredCategory || 'Uitgelicht'}
              meta={[
                ...(featuredCase.client
                  ? [{ icon: '🏢', text: featuredCase.client }]
                  : []),
                ...(featuredCase.industry
                  ? [{ icon: '📊', text: featuredCase.industry }]
                  : []),
              ]}
        ...
        {cases.length > 0 ? (
          ...
          <div className="py-12 text-center text-grey-mid">Geen cases gevonden.</div>
        )}
...
export default CasesArchiveTemplate
