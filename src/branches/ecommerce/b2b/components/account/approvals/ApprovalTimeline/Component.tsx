...
export function ApprovalTimeline({ events }: ApprovalTimelineProps) {
  return (
    <div className="bg-white rounded-xl lg:rounded-2xl p-4 lg:p-5 shadow-sm">
      <h3 className="text-sm font-bold text-navy mb-4">Tijdlijn</h3>
          ...
          return (
            <div key={i} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {!isLast && <div className="w-px flex-1 bg-grey-light my-1" />}
              </div>

              <div className="pb-4">
                <div className="text-sm font-semibold text-navy">{TYPE_LABELS[event.type]}</div>
                {event.user && <div className="text-xs text-grey-mid mt-0.5">Door {event.user}</div>}
                {event.message && (
                  <div className="text-xs text-grey-mid mt-1 italic">&ldquo;{event.message}&rdquo;</div>
                )}
                <div className="text-[10px] text-grey-mid mt-1">{dateStr}</div>
              </div>
            </div>
          )
        })}
