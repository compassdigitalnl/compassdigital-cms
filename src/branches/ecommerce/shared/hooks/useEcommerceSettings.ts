...
interface EcommerceSettingsData {
  ...
  carrierIntegration: {
    provider: 'none' | 'sendcloud' | 'myparcel'
  }
  shippingMethods: any[]
  paymentOptions: any[]
}
...
const DEFAULTS: EcommerceSettingsData = {
  ...
  emailNotifications: {
    enableInvoiceAttachment: false,
    enableTrackingLink: false,
    ...
    _fetchPromise = fetch('/api/globals/e-commerce-settings?depth=1')
      ...
      .then((data) => {
        ...
        const result: EcommerceSettingsData = {
          ...
          emailNotifications: {
            enableInvoiceAttachment: data.emailNotifications?.enableInvoiceAttachment ?? DEFAULTS.emailNotifications.enableInvoiceAttachment,
            enableTrackingLink: data.emailNotifications?.enableTrackingLink ?? DEFAULTS.emailNotifications.enableTrackingLink,
            enablePublicTracking: data.emailNotifications?.enablePublicTracking ?? DEFAULTS.emailNotifications.enablePublicTracking,
          },
          abandonedCart: {
            enabled: data.abandonedCart?.enabled ?? DEFAULTS.abandonedCart.enabled,
            timeoutHours: data.abandonedCart?.timeoutHours ?? DEFAULTS.abandonedCart.timeoutHours,
          },
          carrierIntegration: {
            provider: data.carrierIntegration?.provider ?? DEFAULTS.carrierIntegration.provider,
          },
