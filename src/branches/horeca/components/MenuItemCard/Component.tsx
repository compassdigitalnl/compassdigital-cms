...
export const MenuItemCard: React.FC<MenuItemCardProps> = ({
  ...
  const { title, slug, shortDescription, icon, price, category, allergens, _status } = item
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex items-start justify-between">
        {icon && (
          <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--color-base-100)] text-2xl">
            <span>{icon}</span>
          </div>
        )}
        {category && (
        ...
        {allergenList.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-1.5">
            {allergenList.map((allergen, i) => (
              <AllergenBadge key={i} allergen={allergen} />
            ))}
          </div>
        )}
