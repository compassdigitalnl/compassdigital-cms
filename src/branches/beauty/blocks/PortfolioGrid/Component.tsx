...
export async function PortfolioGridComponent(props: PortfolioGridProps) {
  const { heading, source = 'auto', cases: manualCases, limit = 6, columns = '3' } = props
  ...
  if (source === 'auto') {
    ...
    items = result.docs
  } else if (source === 'manual' && manualCases) {
    items = Array.isArray(manualCases) ? manualCases.filter((c): c is any => typeof c === 'object' && c !== null) : []
  }
