...
import { StylistCard } from '@/branches/beauty/components/StylistCard'
import type { TeamShowcaseProps } from './types'

export async function TeamShowcaseComponent(props: TeamShowcaseProps) {
  const { heading, source = 'auto', members: manualMembers, limit = 6, columns = '3', showBookButton = true } = props
    ...
    <section className="px-4 py-12 md:py-16 lg:py-20">
      <div className="container mx-auto">
        ...
        <div className={`grid ${gridColsClasses[columns || '3']} gap-6`}>
          {members.map((member) => (
            <StylistCard key={member.id} stylist={member} showBookButton={showBookButton} />
          ))}
        </div>
      </div>
    </section>
