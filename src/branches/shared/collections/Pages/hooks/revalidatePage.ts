...
export const revalidatePage: CollectionAfterChangeHook<Page> = ({
  ...
  if (!context.disableRevalidate) {
    try {
      ...
      // Gracefully handle revalidation errors (e.g., when running outside Next.js runtime)
      payload.logger.warn(`[Revalidation] Skipped (not in Next.js runtime): ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  return doc
}
