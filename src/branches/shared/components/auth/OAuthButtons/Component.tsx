...
const providerConfig: Record<OAuthProvider, { label: string; icon: React.ReactNode }> = {
  ...
  microsoft: {
    label: 'Inloggen met Microsoft',
    icon: '🟦', // Placeholder, replace with real Microsoft icon if needed
  },
  apple: {
    label: 'Inloggen met Apple',
    icon: '🍎', // Placeholder, replace with real Apple icon if needed
  },
}
...
export const OAuthButtons: React.FC<OAuthButtonsProps> = ({
  ...
  return (
    <>
      ...
      {showDivider && (
        <div className={styles.divider}>{dividerText}</div>
      )}
    </>
  )
}
