...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`header_navigation_special_items\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`header_navigation_special_items_order_idx\` ON \`header_navigation_special_items\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`header_navigation_special_items_parent_id_idx\` ON \`header_navigation_special_items\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`header\` (
  	...
  	\`navigation_category_settings_show_icons\` integer DEFAULT true,
  	\`navigation_category_settings_show_product_count\` integer DEFAULT true,
  	\`navigation_category_settings_mega_menu_style\` text DEFAULT 'subcategories',
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`header_navigation_special_items\`;`)
